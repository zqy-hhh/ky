...
const data = hebeiMapData as unknown as CityGeoJSON;
const HEAT_VALUES = [860, 680, 520, 610, 570, 760, 470, 490, 650, 720, 560, 700];
...
export default function Heatmap(props: HeatmapProps) {
  ...
  useEffect(() => {
    ...
    const points = data.features.map((el, index) => {
      const [x = 0, y = 0] =
        projection(el.properties.centroid ?? el.properties.center) ?? [];
      return {
        x: Math.floor(x + size / 2),
        y: Math.floor(y + size / 2),
        value: HEAT_VALUES[index % HEAT_VALUES.length],
      };
    });
  ...
  return (
    <group visible={heat && mapMode === "city"} {...args}>
      <mesh ref={refMesh}>
